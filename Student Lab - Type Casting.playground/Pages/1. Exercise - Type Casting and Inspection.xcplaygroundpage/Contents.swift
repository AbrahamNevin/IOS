 ...
 */
let mixedCollection: [Any] = [42, 3.14, "Lookwise", true, 7, 99.99, "Swift", false]
...
for (_, value) in mixedDictionary {
    if let intValue = value as? Int {
        ...
        total2 += doubleValue
    } else if let stringValue = value as? String, let intValueToAdd = Int(stringValue) {
        total2 += Double(intValueToAdd)
    }
}
var newtotal = total + total2
print(newtotal)
/*:
